...
def batched_inference(data, model, tokenizer, prompt_fn, temp, batch_size=4):
    ...
    for i, batch in enumerate(dataloader):
        ...
        for uid, snippet, response_text in zip(batch_ids, batch_snippets, decoded):
            ...
            # save the json every 50 batches in case of crash
        
        if (i + 1) % 50 == 0:
            with open(f"temp_reason_predictions_batch.json", 'w') as f:
...
def run_full_text(model_id, strat_name="chain_of_thought", temp=0.7):
    ...
    os.makedirs(f"output/reason_batch/{model_id}", exist_ok=True)
    os.makedirs(f"output/reason_batch/{model_id}/figures", exist_ok=True)
    os.makedirs(f"output/reason_batch/{model_id}/prediction", exist_ok=True)
    os.makedirs(f"output/reason_batch/{model_id}/bootstrap", exist_ok=True)
    ...
    out_dir = f"output/reason_batch/{model_id}/bootstrap"
...
if __name__ == "__main__":
    ...
    # run_full_text(model_id="Llama-3.1-8B-Instruct", strat_name="structured_reasoning", temp=0)
    # run_full_text(model_id="medgemma-4b-it", strat_name="multi_step_verification", temp=1.0)
    run_full_text(model_id="gpt-oss-20b", strat_name="multi_step_verification", temp=0.3)
    print("Done!")
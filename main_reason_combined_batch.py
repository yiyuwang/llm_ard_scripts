...
def compare_temperatures_with_json(model_id):
    ...
    # data = pd.read_csv("data/set-validation_task-reason_label-6_sample-5_desc-stratified.csv")
    ...
    os.makedirs(f"output/reason_prompting_final/{model_id}", exist_ok=True)
    os.makedirs(f"output/reason_prompting_final/{model_id}/figures", exist_ok=True)
    os.makedirs(f"output/reason_prompting_final/{model_id}/prediction", exist_ok=True)
    ...
    for strat_name, prompt_fn in strategies.items():
        ...
        for temp in temperatures:
            ...
            with open(f"output/reason_prompting_final/{model_id}/prediction/{strat_name}_temp{temp}_predictions.json", 'w') as f:
            ...
            plt.savefig(f"output/reason_prompting_final/{model_id}/figures/cm_{strat_name}_temp{temp}.png")
            plt.close()
    ...
    for metric in metrics:
        ...
        plt.savefig(f"output/reason_prompting_final/{model_id}/figures/temperature_effect_{metric}.png")
        plt.close()
    
    # Save summary
    summary_df.to_csv(f"output/reason_prompting_final/{model_id}/prediction/temperature_strategy_comparison.csv", index=False)
